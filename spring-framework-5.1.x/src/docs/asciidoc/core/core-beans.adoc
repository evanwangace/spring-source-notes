...
The preceding `MyConfig` class is equivalent to the following Spring `<beans/>` XML:
...
----
	public static void main(String[] args) {
		...
		ctx.register(MyConfig.class, OtherConfig.class);
...
assuming that `MyConfig` is declared within the `com.acme` package (or any package
...
----
	<web-app>
		...
		<context-param>
			...
			<param-value>com.acme.MyConfig</param-value>
		</context-param>
...
container. In this series examples, we create a `@Configuration` class named `MyConfig` and
...
`@Configuration` annotation and processes the `@Bean` methods declared in `MyConfig`
properly.
...
NOTE: In `system-test-config.xml` file, the `MyConfig` `<bean/>` does not declare an `id`
...
----
	<beans>
		<!-- picks up and registers MyConfig as a bean definition -->
		<context:component-scan base-package="com.acme"/>
		<context:property-placeholder location="classpath:/com/acme/jdbc.properties"/>
